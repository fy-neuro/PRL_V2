...
unique(sobj$Stage)
sobj <- subset(sobj, Stage %in% c("Early","Non-Early"))
unique(sobj$Stage)
...
# Open a PDF device
pdf(paste0("../plots/netVisual_circle_plot_PD_", job_id, ".pdf"))
...
pdf(paste0("../plots/all_pathways_plots_PD_", job_id, ".pdf"))
...
write_xlsx(df.net, paste0("../output/Ligand-Receptor_interaction_PD_", job_id, ".xlsx"))

# Compute network centrality scores
cellchat <- netAnalysis_computeCentrality(cellchat)

# Generate signaling role heatmap
pdf(paste0("../plots/netAnalysis_signalingRole_heatmap_PD_", job_id, ".pdf"))
